                  ...
                  <form action="{{URL::to('/home/user/changepass')}}" method="POST">
                     ...
                     <div class="row">
                        <div class="form-group col-xl-12">
                           ...
                           <input class="form-control @error('old_pass') is-invalid @enderror" type="password" name="old_pass" value="{{old('old_pass')}}" placeholder="{{trans('labels.enter_old_pass')}}">
                           @error('old_pass')<span class="text-danger"> {{ $message }}</span>@enderror
                        </div>
                        <div class="form-group col-xl-12">
                           <label class="mr-sm-2">{{trans('labels.new_pass')}}</label>
                           <input class="form-control @error('new_pass') is-invalid @enderror" type="password" name="new_pass" value="{{old('new_pass')}}" placeholder="{{trans('labels.enter_new_pass')}}">
                           @error('new_pass')<span class="text-danger"> {{ $message }}</span>@enderror
                        </div>
                        <div class="form-group col-xl-12">
                           <label class="mr-sm-2">{{trans('labels.confirm_pass')}}</label>
                           <input class="form-control @error('confirm_pass') is-invalid @enderror" type="password" name="confirm_pass" value="{{old('confirm_pass')}}" placeholder="{{trans('labels.enter_confirm_pass')}}">
