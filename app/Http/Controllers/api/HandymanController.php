            ...
            if($checkhandyman->is_verified == 1)
            {
               if(Hash::check($request->password,$checkhandyman->password)) 
               {
                  ...
                  $handymandata = array(
                     ...
                     'user_type'=> $data->type,
                     'token'=> $data->token,
                     'image_url' => asset('application/storage/app/public/handyman/'.$data->image)
                  );
      ...
      if(!empty($checkhandyman))
      {
         ...
         $bookings = Booking::join('services','bookings.service_id','services.id')
                           ...
                           ->select('bookings.booking_id','bookings.service_id','bookings.service_name','bookings.duration as booking_duration','bookings.duration_type as duration_type',
                              'bookings.price','bookings.price_type','categories.name as category_name','bookings.status as booking_status','bookings.handyman_accept','bookings.reason',
      ...
      if(!empty($checkbooking)){
         $bookingdata = Booking::join('users as customer', function($query) {
                        ...
                        ->select('bookings.id','bookings.booking_id','bookings.service_id','bookings.service_name','bookings.duration as booking_duration','bookings.duration_type','bookings.price','bookings.price_type','bookings.coupon_code','bookings.discount','bookings.total_amt as total_amount','bookings.payment_type','bookings.date','bookings.time','bookings.status as booking_status','bookings.canceled_by','bookings.note','bookings.handyman_accept','bookings.reason',
                           'customer.id','customer.name as customer_name','customer.email as customer_email','customer.mobile as customer_mobile','bookings.address as customer_address',
                           DB::raw("CONCAT('".asset('application/storage/app/public/profile')."/', customer.image) AS user_image_url"),
   ...
   public function getprofile(Request $request )
   {
      ...
      if(!empty($checkhandyman)){
         $data = array(
            ...
            'image_url' => asset('application/storage/app/public/handyman/'.$checkhandyman->image)
         );
