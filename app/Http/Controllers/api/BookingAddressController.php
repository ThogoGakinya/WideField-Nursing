...
class BookingAddressController extends Controller
{
    public function getaddresses(Request $request)
    {
        ...
        if(!empty($checkuser)){
            ...
            if(!empty($check)){
                $addressdata = BookingAddress::select('id','name','email','mobile','street','landmark','postcode')->where('user_id',$request->user_id)->orderByDesc('id')->paginate(10);
    ...
    public function addaddress(Request $request)
    {
        ...
        $checkaddress = BookingAddress::where(['user_id'=>$request->user_id,'name'=>$request->name,'street'=>$request->street,'landmark'=>$request->landmark,'postcode'=>$request->postcode,'mobile'=>$request->mobile,'email'=>$request->email])->first();
        ...
        $addressdata = BookingAddress::join('users','booking_addresses.user_id','users.id')
                ->select('booking_addresses.id','booking_addresses.name','booking_addresses.street','booking_addresses.landmark','booking_addresses.postcode','booking_addresses.mobile','booking_addresses.email','users.name as user_name')
    ...
    public function editaddress(Request $request)
    {
        ...
        $addressexist = BookingAddress::where('id','!=',$request->address_id)->where(['name'=>$request->name,'street'=>$request->street,'landmark'=>$request->landmark,'postcode'=>$request->postcode,'mobile'=>$request->mobile,'email'=>$request->email])->first();
