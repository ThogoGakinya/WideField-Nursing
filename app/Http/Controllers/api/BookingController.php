...
class BookingController extends Controller
{
   public function paymentmethodlist(Request $request)
   {
      ...
      if($request->type == "booking"){
         $pmdata = PaymentMethods::where('is_available',1)->orderBy('id')->get();
      }
      if($request->type == "wallet"){
         $pmdata = PaymentMethods::where('is_available',1)->where('payment_name','!=','cod')->where('payment_name','!=','wallet')->orderBy('id')->get();
      }
   ...
   public function bookingdetails(Request $request)
   {
      ...
      if(!empty($checkbooking)){
         ...
         $bookingdata = Booking::join('users as providers', function($query) {
                        ...
                        ->select('bookings.booking_id','bookings.service_id','bookings.service_name','bookings.address as booking_address','bookings.duration as booking_duration','bookings.duration_type','bookings.payment_type','bookings.price','bookings.price_type','bookings.coupon_code','bookings.discount','bookings.total_amt as total_amount','bookings.status','bookings.date','bookings.time','bookings.note','bookings.handyman_accept','providers.id as provider_id','providers.name as provider_name','providers.email as provider_email','providers.mobile as provider_mobile','providers.address as provider_address','handyman.name as handyman_name','handyman.email as handyman_email','handyman.mobile as handyman_mobile','handyman.address as handyman_address',
   ...
   public function bookinglist(Request $request)
   {
      ...
      if(!empty($checkuser)){
         ...
         $bookingdata = Booking::select('booking_id','provider_name','payment_type','price','price_type','handyman_accept','status','canceled_by','total_amt as total_amount','date','time')->where('user_id',$request->user_id)->orderByDesc('id')->paginate(10);
