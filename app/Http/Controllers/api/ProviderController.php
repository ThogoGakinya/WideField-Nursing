...
class ProviderController extends Controller
{
   ...
   public function dashboard(Request $request)
   {
      ...
      $bookings = Booking::join('services','bookings.service_id','services.id')
                        ...
                        ->select('bookings.booking_id','bookings.service_id','bookings.service_name','bookings.duration as booking_duration','bookings.duration_type as duration_type',
                           'bookings.price','bookings.price_type','categories.name as category_name',
                           DB::raw("CONCAT('".asset('application/storage/app/public/service')."/', bookings.service_image) AS image_url"))
   ...
   public function datewisebookings(Request $request)
   {
      ...
      if (preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/",$request->date)) {
         $bookings = Booking::join('services','bookings.service_id','services.id')
               ->join('categories','services.category_id','categories.id')
               ->select('bookings.booking_id','bookings.service_id','bookings.service_name','bookings.duration as booking_duration','bookings.duration_type as duration_type',
                  'bookings.price','bookings.price_type','categories.name as category_name','bookings.status',
                  DB::raw("CONCAT('".asset('application/storage/app/public/service')."/', bookings.service_image) AS image_url"))
   ...
   public function bookingdetails(Request $request)
   {
      ...
      if(!empty($checkbooking)){
         $bookingdata = Booking::join('users as providers', function($query) {
                        ...
                        ->select('bookings.*','bookings.booking_id','bookings.service_id','bookings.service_name','bookings.address as booking_address','bookings.duration as booking_duration','bookings.duration_type','bookings.price','bookings.price_type','bookings.payment_type','bookings.coupon_code','bookings.discount','bookings.total_amt as total_amount','bookings.status','bookings.date','bookings.time','bookings.note','providers.name as provider_name','providers.email as provider_email','providers.mobile as provider_mobile','providers.address as provider_address',
                           'handyman.name as handyman_name','handyman.email as handyman_email','handyman.mobile as handyman_mobile','handyman.address as handyman_address',
                           DB::raw("CONCAT('".asset('application/storage/app/public/service')."/', bookings.service_image) AS service_image_url"),
                           DB::raw("CONCAT('".asset('application/storage/app/public/provider')."/', providers.image) AS provider_image_url"),
                           DB::raw("CONCAT('".asset('application/storage/app/public/handyman')."/', handyman.image) AS handyman_image_url"))
                        ->where('booking_id',$request->booking_id)
                        ->first();
         
         $handymandata = User::select('id','name','email','mobile',
                                 DB::raw("CONCAT('".asset('application/storage/app/public/handyman')."/', image) AS image_url"))
                        ->where('type',3)
   ...
   public function services(Request $request)
   {
      ...
      $servicedata =  Service::with('api_rattings')
            ...
            ->select('services.id','services.name as service_name','services.price','services.price_type','services.duration','services.duration_type',
               'categories.name as category_name','services.is_available',
   ...
   public function bookinghistory(Request $request)
   {
      ...
      if(!empty($checkprovider)){
         $bookings = Booking::join('services','bookings.service_id','services.id')
                        ->join('categories','services.category_id','categories.id')
                        ->select('bookings.id','bookings.booking_id','bookings.service_id','bookings.service_name','bookings.duration as booking_duration','bookings.duration_type as duration_type',
                        'bookings.date','bookings.time','bookings.price','bookings.price_type','bookings.status as booking_status','categories.name as category_name','bookings.payment_type','bookings.handyman_id','bookings.handyman_accept','bookings.reason',
                           DB::raw("CONCAT('".asset('application/storage/app/public/service')."/', bookings.service_image) AS image_url"))
                        ->where('bookings.provider_id',$request->provider_id)
