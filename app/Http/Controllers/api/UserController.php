...
class UserController extends Controller
{
        ...
        if (!empty($checkuser)) {
            ...
            if ($checkuser->otp == $request->otp) {
                ...
                $data = array(
                    ...
                    'image_url' => asset('application/storage/app/public/profile/'.$checkuser->image),
                );
                ...
                if($u->is_verified == 1)
                {
                    if(Hash::check($request->password,$u->password)) 
                    {
                        ...
                        $userdata = array(
                            ...
                            'wallet'=> $data->wallet,
                            'user_type'=> $data->type,
                            'image_url' => asset('application/storage/app/public/profile/'.$data->image)
                        );
    ...
    public function addhelp(Request $request)
    {
        ...
        if($request->first_name == ""){
            return response()->json(['status'=>0,'message'=>trans('messages.enter_fname')],200);
        }
        if($request->last_name == ""){
            return response()->json(['status'=>0,'message'=>trans('messages.enter_lname')],200);
        }
        if($request->email == ""){
            return response()->json(['status'=>0,'message'=>trans('messages.enter_email')],200);
        }
        ...
        if($request->subject == ""){
            return response()->json(['status'=>0,'message'=>trans('messages.enter_subject')],200);
        }
        if($request->message == ""){
            return response()->json(['status'=>0,'message'=>trans('messages.enter_message')],200);
        }

        $help = new Help();
        $help->user_id = $request->user_id;
        $help->fname = $request->first_name;
        $help->lname = $request->last_name;
