...
class TimingController extends Controller
{
    ...
    {
        $timingdata = Timing::where('provider_id',Auth::user()->id)->get();
        return view('timing.show',compact('timingdata'));
    }
    public function edit(Request $request)
    {
        ...
        foreach($day as $key => $no)
        {
            ...
            Timing::where('provider_id',Auth::user()->id)->where('day', $no)->update($input);
        }
